    ...
    function render() {
      ...
      if (state.over || state.won) {
        ...
        if (!state.started) {
          ...
          ctx.font = isMobileLike ? '15px Inter, sans-serif' : '18px Inter, sans-serif';
          ctx.fillText(isMobileLike ? 'स्वाइप करून खेळ चालू करा' : 'बाणाचे बटण दाबून खेळ चालू करा', canvas.width / 2, canvas.height - (isMobileLike ? 56 : 72));
          ctx.fillStyle = '#d7e4ff';
          ctx.font = isMobileLike ? '12px Inter, sans-serif' : '14px Inter, sans-serif';
          ctx.fillText(isMobileLike ? 'डबल टॅप = आगगोळा' : 'Space = आगगोळा   •   P = थांबा', canvas.width / 2, canvas.height - (isMobileLike ? 30 : 42));
        } else {
          ctx.fillStyle = '#fff';
